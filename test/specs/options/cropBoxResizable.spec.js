describe('cropBoxResizable (option)', () => {
  it('should be resizable by default', (done) => {
    ...
    const queryClasses = '.cropper-line, .cropper-point';
    const cropper = new Cropper(image, {
      ready() {
        Array.from(cropper.cropper.querySelectorAll(queryClasses)).forEach((handler) => {
  ...
  //     ready() {
  //       Array.from(cropper.cropper.querySelectorAll('.cropper-line, .cropper-point'))
  // .forEach((handler) => {
